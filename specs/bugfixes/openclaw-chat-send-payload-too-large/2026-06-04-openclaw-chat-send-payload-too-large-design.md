...
因此，多张图片在单张都不超过 30 MiB 的情况下，合并后的 JSON/WebSocket payload 仍可能超过 OpenClaw 网关上限。
...
3. 用户提示应说明“本次消息过大”，并给出可操作建议：减少附件数量或拆分提交。
4. 提示中应说明单次消息上限约为 30 MB，但避免承诺精确文件大小等价于 30 MB。
...
```typescript
const OPENCLAW_CHAT_SEND_PAYLOAD_LIMIT_BYTES = 30 * 1024 * 1024;
const OPENCLAW_CHAT_SEND_PAYLOAD_SAFETY_MARGIN_BYTES = 512 * 1024;
...
```text
本次消息过大，AI 引擎无法接收。请减少附件数量，或拆分为多次提交。（单次消息需小于约 30MB）
```

如果调用方能提供估算大小，推荐更具体的文案：

```text
本次消息约 {size}，超过 AI 引擎单次接收上限（约 {limit}）。请减少附件数量，或拆分为多次提交。
```

英文推荐文案：

```text
This message is too large for the AI engine to receive. Reduce the number of attachments or split it into multiple messages. (Each message must be under about 30 MB.)
```
...
2. 使用 `30 MiB - 512 KiB` 的 safe limit。
3. 保留 `1009` / `Max payload size exceeded` 兜底错误分类。

### 6.2 “约 30MB”与 MiB/MB 差异

代码内部使用 30 MiB，用户文案使用约 30 MB。严格来说两者不同。
...
3. 512 KiB 安全余量相对 30 MiB 较小，可接受。
