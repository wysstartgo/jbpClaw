...
本技能是 `xiaohu-wechat-format` 的封面子能力独立入口。配置文件仍复用相邻的 `xiaohu-wechat-format/cover/`：
- `../xiaohu-wechat-format/cover/config.json`：真实配置（从 `config.example.json` 复制后填写）
- `../xiaohu-wechat-format/cover/config.example.json`：示例配置
...
调用生图脚本，**必须传 `--config` 指向 `xiaohu-wechat-format/cover/config.json`**：
```bash
python3 <SKILLs>/xiaohu-wechat-format/scripts/generate.py \
  --config <SKILLs>/xiaohu-wechat-format/cover/config.json \
  --prompt-file <目标目录>/prompt.md \
  --out <目标目录>/cover.jpg
```

> `<SKILLs>` 是 QingShuClaw 的技能安装根目录。不要在 `xiaohu-wechat-cover` 下另建一份脚本或配置，避免与 `xiaohu-wechat-format` 配置漂移。
