---
name: oceanengine-account-auth-funds
description: 适用于 OceanEngine 中与“账户、鉴权与资金”相关的需求，优先使用本域 Tool-Range 白名单处理任务，而不是直接暴露全量 AD MCP 工具。
---
...
覆盖 OAuth、用户身份、广告主基础信息、预算、账户余额、钱包与资质。

当前域工具数：`41`

典型场景：
- 查询广告主基本信息、预算、余额、流水
- 检查 Access-Token / App Access Token 相关调用前置
- 核对账户资质、钱包预算与安全积分

示例任务：
- 帮我查询这个广告主的基础信息、预算和余额，并标出异常点。
- 帮我判断当前 Access-Token 是否足够支持后续调用，并列出缺失前置。
- 帮我梳理账户资质、钱包与安全积分状态，给出风险摘要。

非目标：
- 不负责代理商侧转账、退款、组织间资金流转
- 不负责项目、广告、创意、落地页和线索操作
