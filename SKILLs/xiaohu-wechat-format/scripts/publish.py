...
# ── 微信 API ─────────────────────────────────────────────────────────
def _iter_wechat_accounts(config):
    accounts = config.get("wechat_accounts") or config.get("wechatAccounts") or []
    if isinstance(accounts, dict):
        for account_id, account in accounts.items():
            if isinstance(account, dict):
                merged = {"id": account_id, **account}
                yield merged
        return
...
def resolve_wechat_config(config, account_id=None):
    """解析公众号账号配置，兼容旧版单账号 wechat 配置。"""
    legacy = config.get("wechat", {}) or {}
    accounts = list(_iter_wechat_accounts(config))
    ...
    if account_id:
        ...
        available = ", ".join(
            str(account.get("id") or account.get("name") or "<unnamed>") for account in accounts
        )
        print(f"错误: 未找到公众号账号配置: {account_id}")
        if available:
            print(f"  可用账号: {available}")
        sys.exit(1)

    if legacy.get("app_id") and legacy.get("app_secret"):
        return legacy

    if len(accounts) == 1:
        return {**legacy, **accounts[0]}

    if len(accounts) > 1:
        print("错误: 检测到多个公众号账号，请用 --account 指定目标账号")
        print(
            "  可用账号: "
            + ", ".join(str(account.get("id") or account.get("name") or "<unnamed>") for account in accounts)
        )
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("--config", default=str(DEFAULT_CONFIG_PATH),
                        help="配置文件路径（默认读取 skill 目录下 config.json）")
    parser.add_argument("--account",
                        help="公众号账号 ID；配置多个 wechat_accounts 时必须指定")
    ...
    wechat_config = resolve_wechat_config(config, args.account)
    ...
    author = args.author if args.author is not None else wechat_config.get("author", "")
    print(f"标题: {title}")
    print(f"作者: {author}")
    if args.account or wechat_config.get("id") or wechat_config.get("name"):
        print(f"账号: {wechat_config.get('name') or wechat_config.get('id') or args.account}")

    # ── 4. 获取 token ────────────────────────────────────────────────
    print(f"\n获取 access_token...")
    token = get_access_token(wechat_config)
    print("✓ token 获取成功")
