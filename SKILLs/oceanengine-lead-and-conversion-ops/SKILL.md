---
name: oceanengine-lead-and-conversion-ops
description: 适用于 OceanEngine 中与“线索、转化与承接链路”相关的需求，优先使用本域 Tool-Range 白名单处理任务，而不是直接暴露全量 AD MCP 工具。
---
...
覆盖线索、表单、券码、智能电话、微信号包、转化目标与事件管理。

当前域工具数：`60`

典型场景：
- 管理表单、飞鱼线索、券码、电话和微信号包
- 查询和推送转化目标、RTA、事件与起量任务
- 打通线索回传与转化追踪

示例任务：
- 帮我查询当前表单、线索和承接资产的总体情况。
- 帮我检查转化目标、RTA 和事件配置是否齐全。
- 帮我梳理券码、电话和微信号包的承接链路状态。

非目标：
- 不负责素材上传、报表导出或代理商财务结算
- 不负责商品库与 DPA 商品管理
